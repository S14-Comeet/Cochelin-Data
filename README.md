...
```mermaid
flowchart TB
    subgraph Collection["🗂️ Data Collection"]
        A1["Selenium + Chrome"] --> A2["네이버 지도 크롤링"]
        B1["Kaggle CSV"] --> B2["coffee_clean.csv"]
    end

    subgraph Processing["⚙️ Data Processing"]
        C1["Pandas"] --> C2["CSV 파싱/정제<br/>데이터 변환<br/>메뉴-원두 매핑"]
        D1["LangChain + GPT-4o-mini"] --> D2["RAG Context<br/>(flavors_rag.json)"]
        D2 --> D3["향미 추출 & 매핑"]
    end

    subgraph Export["📤 Data Export"]
        E1["Python Script"] --> E2["CSV → SQL 변환<br/>(INSERT문 생성)"]
        F1["MySQL"] --> F2["데이터베이스 Import<br/>(schema + data)"]
    end
...
flowchart TB
    ...
    subgraph 크롤링["1️⃣ 크롤링"]
        N --> C1["1_crawl_cafes.py"]
        C1 --> S1["data/raw/stores.csv<br/>data/raw/menus.csv"]
    end

    subgraph 원두처리["2️⃣ 원두 처리"]
        K --> KC["data/raw/coffee_clean.csv"]
        KC --> C2["2_process_beans.py"]
        RAG["data/debug/flavors_rag.json<br/>(SCA Flavor Wheel)"] -.-> C2
        C2 --> B1["data/beans.csv<br/>data/bean_flavor_notes.csv"]
    end

    subgraph 전처리["3️⃣ 전처리"]
        S1 --> C3["3_preprocess_for_db.py"]
        B1 --> C3
        C3 --> F1["data/final/roasteries.csv<br/>data/final/stores.csv<br/>data/final/beans.csv"]
    end

    subgraph 매핑["4️⃣ 매핑"]
        F1 --> C4["4_map_menu_beans.py"]
        C4 --> F2["data/final/menu_bean_mappings.csv"]
    end

    subgraph SQL["5️⃣ SQL 생성"]
        F2 --> C5["5_generate_sql.py"]
        C5 --> OUT["sql/data_import.sql"]
    end
```
