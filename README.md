...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│   [Selenium + Chrome]              [Kaggle CSV]                         │
│         │                               │                               │
│         ▼                               ▼                               │
│   네이버 지도 크롤링              coffee_clean.csv                      │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
                    │                               │
                    ▼                               ▼
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│   [Pandas]                        [LangChain + GPT-4o-mini]             │
│      │                                   │                              │
│      ▼                                   ▼                              │
│   CSV 파싱/정제               ┌─────────────────────┐                   │
│   데이터 변환                 │    RAG Context      │                   │
│   메뉴-원두 매핑              │  (flavors_rag.json) │                   │
│                               │         +           │                   │
│                               │   LLM Extraction    │                   │
│                               └─────────────────────┘                   │
│                                         │                               │
...
│         │                            │                                  │
│         ▼                            ▼                                  │
│   CSV → SQL 변환              데이터베이스 Import                       │
│   (INSERT문 생성)             (schema + data)                           │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
1_crawl_cafes.py                  data/raw/coffee_clean.csv
     │                                    │
     ▼                                    ▼
data/raw/stores.csv              2_process_beans.py (GPT-4o-mini + RAG)
data/raw/menus.csv                    ↑       │
                                      │       │
                           data/debug/flavors_rag.json
                           (SCA Flavor Wheel 컨텍스트)
     │                                    ▼
     │                            data/beans.csv
